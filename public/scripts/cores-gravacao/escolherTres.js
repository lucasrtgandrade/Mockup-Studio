...
function updateDisplay() {
    ...
    corDigitada.forEach(color => {
        if (color && cores.length < 4 && !cores.includes(color + ' cliente')) {
    ...
    if (cores.length > 3) {
        ...
        document.getElementById('inputCorCliente').value = cores.slice(0, 4).join(' ');
        return;
    }
...
function ativarChip(chipElement, nomeCor) {
    ...
    if (!isActive && selectedCores.length < 4) {
